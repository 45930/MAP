...
export class Poll extends SmartContract {
	...
	@method
	castVote(vote: Ballot) {
		const unpackedVote1 = new PartialBallot(vote.partial1).toBigInts();
		const unpackedVote2 = new PartialBallot(vote.partial2).toBigInts();

		let voteSum = UInt32.from(0);
		for (let i = 0; i < PartialBallot.l; i++) {
			voteSum = voteSum.add(Number(unpackedVote1[i]));
		}
		for (let i = 0; i < PartialBallot.l; i++) {
			voteSum = voteSum.add(Number(unpackedVote2[i]));
		}
